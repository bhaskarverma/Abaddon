package me.apollyon.abaddon.abaddon.ast
...
fun AbaddonFileContext.toAst(considerPosition: Boolean = false) : AbaddonFile = AbaddonFile(this.line().map { it.statement().toAst(considerPosition) }, toPosition(considerPosition))
...
class AbaddonParseTreeToAstMapper : ParseTreeToAstMapper<AbaddonFileContext, AbaddonFile> {
    override fun map(parseTreeNode: AbaddonFileContext): AbaddonFile = parseTreeNode.toAst()
}
